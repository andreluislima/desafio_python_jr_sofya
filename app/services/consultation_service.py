...
def padronizar_genero(value: str) -> str:
    value = (value or "").strip().lower()

    genero_feminino = {"f", "female", "woman", "feminino", "fem", "mulher"}
    genero_masculino = {"m", "male", "man", "masculino", "masc", "homem"}

    if value in genero_feminino:
        return "female"
    elif value in genero_masculino:
        return "male"
    else:
        return "other"

def processar_consulta(entrada_consulta: ConsultationIn) -> ConsultationOut:
    idade = calcular_idade(entrada_consulta.patient.birth_date, entrada_consulta.appointment.date)
    ...
    resumo_prontuario = (
        f"Paciente {entrada_consulta.patient.name}, {idade} anos, sexo {genero}. Queixa principal: {entrada_consulta.appointment.complaint}"
    )
